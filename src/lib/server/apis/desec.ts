...
import { Result, Err, Ok } from "@oxi/result"
...
type RRSetResponse = { created: string, domain: string, subname: string, name: string, type: string, records: string[], ttl: number, touched: string } | null
...
export async function setRRSet(subdomain: string, records: string[]): Promise<Result<null, string>> {
