<script lang="ts">
	...
	let timeSinceLastPush = $derived(
		domain.last_push ? clock.getTime() - domain.last_push.getTime() : undefined
	)
	...
	<ul class="list bg-base-100 rounded-box border">
		...
		{#each $form.NS_records as _, i}
			<label class="border list-row group flex flex-col w-full">
				<div class="join">
					<input
						class="input w-full peer input-ghost"
						class:border-error={$errors.NS_records?.[i]}
						bind:value={() => $form.NS_records[i], (t) => ($form.NS_records[i] = t.toLowerCase())}
					...
					<button
						...
						class="peer-focus:opacity-100 peer-focus:pointer-events-auto focus:opacity-100 focus:pointer-events-auto btn opacity-0 transition-all pointer-events-none duration-200"
						>x</button
					>
				</div>
...
<span
	>last push to DNS: {#if domain.last_push}{domain.last_push}{:else}<span class="text-accent"
			>(never)</span
		>{/if}</span
>

<p class="text-accent text-sm">
	this means that was the last time that the values above were actually sent to the DNS servers, to
	send them again press the button below. you can only push every 15 minutes
</p>
